    ...
    def build(self):
        ...
        for name, option in OPTIONS.items():
            ...
            if option['type'] == 'checkbox':
                ...
                self.option_layout.addWidget(QtWidgets.QLabel(option['label']))
                widget = QtWidgets.QComboBox()
                idx = 0
                for item in option['options']:
                    widget.addItem(item['label'])
                    if option['default'] == item['values'] or option['default'] == item['values']:
                        widget.setCurrentIndex(idx)
                    idx += 1
            elif option['type'] == 'line':
                widget = QtWidgets.QLineEdit(option.get('default') or '')
