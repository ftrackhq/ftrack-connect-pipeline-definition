# :coding: utf-8
# :copyright: Copyright (c) 2014-2020 ftrack

import os
import traceback

import maya.cmds as cmds

from ftrack_connect_pipeline import utils
from ftrack_connect_pipeline_maya import plugin
import ftrack_api

def extract_load_mode_component_name(data):
    for step_result in data:
        if step_result['type'] != 'component':
            continue
        for stage_result in step_result['result']:
            if stage_result['name'] == 'importer':
                for plugin_result in stage_result['result']:
                    load_mode = plugin_result.get('options',{}).get('load_mode')
                    if load_mode:
                        return load_mode, step_result['name']
    return None

class MayaFinalizePlugin(plugin.LoaderFinalizerMayaPlugin):
    plugin_name = 'maya_finalize'

    def run(self, context_data=None, data=None, options=None):
        import json
        self.logger.warning('MayaFinalizePlugin({},{},{})'.format(
            json.dumps(context_data, indent=2),
            json.dumps(data, indent=2),
            json.dumps(options, indent=2)
        ))
        result = {}
        message = 'No Maya finalization needed.'

        load_mode, filename = extract_load_mode_component_name(data)

        work_path_base = os.environ.get('FTRACK_CONNECT_WORK_PATH')
        work_path = None

        ctx = self.session.query('Context where id={}'.format(utils.get_current_context_id())).one()

        structure_names = [
            item['name']
            for item in ctx['link']
        ]

        if work_path_base:
            # Build path down to context
            work_path = os.sep.join([work_path_base] + structure_names + ['work'])
        else:
            # Try to query location system (future)
            try:
                location = self.session.pick_location()
                work_path = location.get_filesystem_path(ctx)
            except:
                self.logger.debug(traceback.format_exc())
                # Ok, use default location
                work_path_base = os.path.join(
                    os.path.expanduser('~'),
                    'Documents',
                    'ftrack_work_path'
                )
                # Build path down to context
                work_path = os.sep.join([work_path_base] + structure_names)

        if work_path is not None:
            if not os.path.exists(work_path):
                os.makedirs(work_path)
            if not os.path.exists(work_path):
                return (False,  {'message': 'Could not create work directory: {}!'.format(work_path)})
            work_path = os.path.join(work_path, filename)
            # Save Maya scene to this path
            cmds.file(rename=work_path)
            cmds.file(save=True)
            message = 'Saved opened Maya scene @ "{}"'.format(work_path)
            result['work_path'] = work_path

        user_data = {'message': message}
        return (result, user_data)


def register(api_object, **kw):
    if not isinstance(api_object, ftrack_api.Session):
        # Exit to avoid registering this plugin again.
        return
    plugin = MayaFinalizePlugin(api_object)
    plugin.register()