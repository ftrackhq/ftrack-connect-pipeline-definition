...
class UnrealGeometryPublisherValidatorPlugin(
    plugin.UnrealPublisherValidatorPlugin
):
    plugin_name = 'unreal_geometry_publisher_validator'

    def run(self, context_data=None, data=None, options=None):
        '''Return true if all the collected Unreal node supplied with *data* are geometry'''
        if not data:
            return False
...
def register(api_object, **kw):
    if not isinstance(api_object, ftrack_api.Session):
        ...
        return
    plugin = UnrealGeometryPublisherValidatorPlugin(api_object)
    plugin.register()
