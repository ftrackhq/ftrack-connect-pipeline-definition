...
class UnrealThumbnailPublisherExporterPlugin(plugin.UnrealPublisherExporterPlugin):
    plugin_name = 'unreal_thumbnail_publisher_exporter'

    def run(self, context_data=None, data=None, options=None):
        '''Export a Unreal thumbnail to a temp file for publish'''
...
def register(api_object, **kw):
    if not isinstance(api_object, ftrack_api.Session):
        ...
        return
    plugin = UnrealThumbnailPublisherExporterPlugin(api_object)
    plugin.register()
