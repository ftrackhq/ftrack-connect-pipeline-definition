...
class UnrealRootOpenerContextOptionsWidget(BaseOptionsWidget):
    '''Unreal opener root context plugin widget'''
    ...
    def __init__(
        ...
        super(UnrealRootOpenerContextOptionsWidget, self).__init__(
            parent=parent,
            session=session,
            data=data,
            name=name,
    ...
    def post_build(self):
        '''Post build hook.'''
        super(UnrealRootOpenerContextOptionsWidget, self).post_build()
...
class UnrealRootOpenerContextOptionsPluginWidget(
    plugin.OpenerContextPluginWidget
):
    '''Opener context widget enabling selection of root ftrack context'''

    plugin_name = 'unreal_root_opener_context'
    widget = UnrealRootOpenerContextOptionsWidget
...
def register(api_object, **kw):
    if not isinstance(api_object, ftrack_api.Session):
        ...
        return
    plugin = UnrealRootOpenerContextOptionsPluginWidget(api_object)
    plugin.register()
